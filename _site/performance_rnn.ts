...
let sketch = function (p: p5) {
  ...
  let soundOn = false;
  if (p.mousePressed) {soundOn = true};
  p.draw = function () {
    p.background(0, 12);
    p.textAlign(p.RIGHT); p.textSize(18); p.textFont('Helvetica'); p.fill(200, 200, 255);
    ...
    p.textAlign(p.LEFT); p.textSize(20);
    if (soundOn) {
      p.text("Click anywhere and the music will play", 20, 140)
    }
    let xoff = 0;
